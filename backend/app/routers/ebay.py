...
def _make_sku(card: "Card", item: InventoryItem) -> str:
    ...
    card_num = re.sub(r"[^a-z0-9]+", "-", card.card_number.lower()).strip("-") or "0"
    return f"{card.game}-{card.set_id}-{card_num}-{variant_slug}-{item.condition}"
...
async def create_location(key: str = "home", country: str = "US"):
    """Create an eBay merchant location (required before publishing listings)."""
    try:
        result = await ebay_client.create_merchant_location(key=key, country=country)
        return {"status": "created", "key": key, "result": result}
    except RuntimeError as e:
...
async def list_on_ebay(item_id: int, db: Session = Depends(get_db)):
    ...
    location_key = await ebay_client.get_merchant_location_key()
    ...
    if item.listed_on_ebay and item.ebay_offer_id and item.ebay_inventory_sku:
        ...
        # New listing — create offer (or reuse existing) and publish
        try:
            offer_resp = await ebay_client.create_offer(sku=sku, price=float(item.ebay_price), merchant_location_key=location_key)
            offer_id = offer_resp["offerId"]
        except RuntimeError as e:
            if "already exists" not in str(e):
                raise HTTPException(status_code=500, detail=str(e))
            existing = await ebay_client.get_offers_for_sku(sku)
            if not existing:
                raise HTTPException(status_code=500, detail=f"Offer exists but could not retrieve it: {e}")
...
async def list_set_on_ebay(game: str, set_id: str, db: Session = Depends(get_db)):
    ...
    # Check if already listed (any item has a per-item offer ID)
    already_listed = any(i.ebay_offer_id for i in items)
    ...
    if already_listed:
        # Update qty/price on each item's existing offer
        for item in items:
            ...
            item.updated_at = now
        db.commit()
        return {"status": "updated", "group_key": group_key, "cards_updated": len(items)}
    else:
        # Create one offer per SKU, publish the first — eBay creates the full multi-variation listing
        first_offer_id = None
        for item in items:
            sku = _make_sku(item.card, item)
            try:
        ...
        await ebay_client.publish_offer(first_offer_id)
        ...
        for item in items:
            ...
            item.updated_at = now
        db.commit()
        return {"status": "listed", "group_key": group_key, "cards_listed": len(items)}
...
async def delist_set_from_ebay(game: str, set_id: str, db: Session = Depends(get_db)):
    ...
    # Withdraw each item's individual offer
    seen_offer_ids: set[str] = set()
    for item in items:
        oid = item.ebay_offer_id or item.ebay_group_offer_id
        if oid and oid not in seen_offer_ids:
            await ebay_client.delist_offer(oid)
            seen_offer_ids.add(oid)

    now = datetime.now(timezone.utc)
    for item in items:
        item.listed_on_ebay = False
        item.ebay_offer_id = None
